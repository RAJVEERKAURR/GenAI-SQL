...
nl_to_sql=prompt | llm

# Step 4: Invoke the chain

user_question = input("Enter your question about Bollywood movies: ")
print("\nConverting to SQL...")
response = nl_to_sql.invoke({"question": user_question})

sql_query=response.content
print(sql_query)

#Steop 5: Pass above sql query to sqlite database and execute it.

db_conn = sqlite3.connect('bollywood_movie.db')

# Function to execute SQL and return results in a Pandas DataFrame
def execute_sql_query(sql_query, db_conn):
    # Execute the SQL query
    result = pd.read_sql_query(sql_query, db_conn)
    return result

result_df = execute_sql_query(sql_query, db_conn)
print(result_df)